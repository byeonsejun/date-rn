...
export const useWeather = () => {
    ...
    async (districtName: string) => {
      ...
      try {
        const [today, forecast] = await Promise.all([
          fetchRealTimeWeather(found.lat, found.lon),
          fetchForecastWeather(found.lat, found.lon),
        ]);
  ...
  useEffect(() => {
    ...
    setNeedsGps(false);
    void getSelectLocation(location);
  }, [location, allDistrictInfo.length, getSelectLocation]);
