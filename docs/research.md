...
## RN 이슈(미해결): 두드림길 `filterDodreamgilByDistrict` 데이터 손실 + POI 원본 데이터 정합성 (2026-07-10, i18n 3단계 검증 중 발견)

> **아직 수정하지 않음.** i18n 3단계(장소 표시 다국어화) 작업과 무관한 선재 버그라 관심사 분리 원칙상 별도 커밋/작업으로 분리한다. 수정 시 콤마로 묶인 다중 구 그룹 파싱 + 중복 처리 설계가 필요해 별도 작업으로 볼 것.

### 1) `filterDodreamgilByDistrict`가 `.find()`로 첫 매치만 취해 데이터 대량 누락

**위치**

- `src/entities/poi/lib/filter.ts` — `filterDodreamgilByDistrict`

**증상**

- `dodreamgil.json`은 그룹의 `location` 필드가 `"강동구"`처럼 단일 구인 경우도 있고, `"강북구,도봉구,성북구,은평구,종로구"`처럼 콤마로 여러 구를 묶은 경우도 있다.
- `groups.find((g) => g.location.includes(district))`는 **배열에서 가장 먼저 매치되는 그룹 하나만** 반환한다. 콤마로 묶인 그룹이 배열상 단일 구 그룹보다 먼저 나오면, 그 뒤에 있는 진짜 단일 구 그룹은 영영 조회되지 않는다.

**영향 (실사용 데이터 손실)**

- 서울 25구 기준으로 순회했을 때, 아래 12개 그룹(총 388건)이 `filterPoisByDistrict`/두드림길 추천·지도 표시 경로에서 전혀 노출되지 않음:
  - `종로구` 111건, `성북구` 64건, `관악구,금천구` 75건, `중구` 28건, `강동구,송파구` 17건, `마포구,은평구` 15건, `강남구,서초구` 14건, `마포구,서대문구` 14건, `강북구,도봉구,성북구,은평구,종로구` 18건, `강서구,구로구,금천구,영등포구` 12건, `광진구,중랑구` 12건, `구로구,양천구` 8건
- 특히 `종로구`(111건)·`성북구`(64건)·`중구`(28건)는 **단일 구 전용 그룹 자체가 통째로 숨겨지는** 심각한 케이스.
- `dodreamgil.json` 원본은 총 1443건(35개 그룹)인데, 25구 순회 기준으로는 1121건만 노출됨(차이 322건). `CPI_NAME_en`(titleEn) 누락 때문이 아니라 **전량 이 매칭 로직 때문**(원본 JSON은 `_en` 필드 누락 0건, i18n 3단계 완결과 무관).

**계획 (수정 시 필요한 설계)**

- 콤마로 묶인 `location` 문자열을 분리해 구별로 그룹을 인덱싱하거나, `.find()` 대신 매치되는 **모든** 그룹을 수집(`.filter()`)하도록 변경.
- 한 POI가 여러 구에 동시 소속될 수 있으므로(예: 강동구·송파구 경계), 구별 조회 시 **중복 노출**을 허용할지 dedupe할지 정책 결정 필요.
- park.json에도 유사하게 `p_park`(공원명) 기준 매칭이지만 구조가 단순(구 1:1)이라 이 버그의 영향은 없음 — dodreamgil.json 전용 이슈.

### 2) POI 원본 데이터 정합성 이슈 (참고용, 같은 검증 세션에서 발견)

- **park.json**: 25개 서울 자치구 목록에 속하지 않는 그룹 3건 존재 — `과천시`(1건, 서울 외 지역), `기타`(1건), `강북구 등`(1건, "강북구"와 문자열이 달라 정확 매칭 안 됨). `_en` 번역 필드 자체는 누락 없음.
- **culturalSpace.json**: `종로구` 그룹의 "한국미술관" 항목 좌표가 `X_COORD: "37.5718961547884~2"`처럼 오타(`~2`)가 섞여 있어 `filter.ts`의 `toNumber()`가 `NaN` 처리 → 좌표 기반 필터에서 조용히 탈락. `FAC_NAME_en`은 있음, 좌표 데이터 오류.

**관련 파일**

- `src/entities/poi/lib/filter.ts`
- `src/entities/poi/model/data/park.json`, `culturalSpace.json`, `dodreamgil.json`
